...
class bam:
    ...
    """
    def __init__(self,bam_file,ref_file,prefix,threads=4):
    ...
    def call_variants(self,caller,bed_file=None):
        self.vcf_file = "%s.vcf.gz" % (self.prefix) if bed_file else "%s.targets.vcf.gz" % (self.prefix)

    def call_variants(self,caller,gff_file=None,bed_file=None,call_method="optimise",min_dp=10,max_dp=None,threads=4,mixed_as_missing=False,low_dp_as_missing=True,af=0.0,whole_genome=False,platform="Illumina",**kwargs):
        add_arguments_to_self(self,locals())
