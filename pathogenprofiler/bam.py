...
class bam:
	...
	def call_variants(self,gff_file=None,bed_file=None,call_method="optimise",min_dp=10,threads=4,mixed_as_missing=False,af=0.0,**kwargs):
		...
		if gff_file and filecheck(gff_file):
			...
			self.ann_bcf_file = "%(prefix)s.csq.bcf" % vars(self)
			run_cmd("bcftools csq -p m -f %(ref_file)s -g %(gff_file)s %(variant_bcf_file)s -Ob -o %(ann_bcf_file)s" % vars(self))
	...
	def load_genome_cov(self,bed_file=None):
		...
		if bed_file:
			samtools_cmd = "samtools view -bL %s %s | samtools depth -aa --reference %s -" % (bed_file,self.bam_file,self.ref_file)
		else:
