        ...
        if split_indels:
            with TempFilePrefix() as tmp:
                ...
                if bam_for_phasing:
                    ...
                    self.phasing_bam = ""
                run_cmd("bcftools view -a %(filename)s | bcftools view -c 1 | right_align_deletions.py - %(ref_file)s %(gff_file)s - |  combine_vcf_variants.py --ref %(ref_file)s --gff %(gff_file)s %(phasing_bam)s | %(rename_cmd)s snpEff ann -c %(snpeff_config)s -noLog -noStats %(db)s - %(re_rename_cmd)s | bcftools sort -Oz -o %(vcf_csq_file)s" % vars(self))
