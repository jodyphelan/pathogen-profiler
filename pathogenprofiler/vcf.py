    ...
    def run_snpeff(self,db,ref_file,gff_file,rename_chroms = None, split_indels=True, bam_for_phasing=None):
        ...
        if split_indels:
            with TempFilePrefix() as tmp:
                ...
                run_cmd("bcftools view -c 1 -a %(filename)s | bcftools view -v indels | right_align_deletions.py - %(ref_file)s - | %(rename_cmd)s snpEff ann %(snpeff_data_dir_opt)s -noLog -noStats %(db)s - %(re_rename_cmd)s | bcftools sort -Oz -o %(tmp_file2)s && bcftools index %(tmp_file2)s" % vars(self))
                run_cmd("bcftools view -c 1 -a %(filename)s | bcftools view -v other | right_align_deletions.py - %(ref_file)s - | %(rename_cmd)s snpEff ann %(snpeff_data_dir_opt)s -noLog -noStats %(db)s - %(re_rename_cmd)s | bcftools sort -Oz -o %(tmp_file3)s && bcftools index %(tmp_file3)s" % vars(self))
